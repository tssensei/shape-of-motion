...
\postersubsection{Pixels are too many degrees of freedom}

Tracking $P$ pixels independently produces a coupled displacement vector
$d(t)\in\mathbb R^{2P}$. Modal analysis replaces these dense degrees of freedom
with a compact basis:
\equationbox{PosterSage}{
  d(t)\approx\Phi q(t)=\sum_{k=1}^{K}\phi_kq_k(t),
  \qquad K\ll 2P.
}
Each $\phi_k$ is a coherent spatial motion pattern and $q_k(t)$ is its temporal
activation. Under small linear motion, the modal coordinates approximately
decouple and can be analyzed independently. Only a few modes usually dominate
the visible response: they are resonantly excited, lightly damped, and clearly
...
\postersubsection{Why the frequency domain?}

Time-domain pixel motion mixes many modes. Because each mode has a natural
frequency, the temporal Fourier transform separates their responses:
\equationbox{PosterSage}{
  \widehat d(f)=\sum_t d(t)e^{-i2\pi ft},
  \qquad
  \widehat d(f_k)\propto\phi_k^{\mathrm{2D}}.
}
Near a resonant peak $f_k$, one spatial mode often dominates. Davis, Chen, and
Durand use this observation to recover projected complex 2D modes from ordinary
...
Each frequency slice is a candidate spatial pattern. Strong resonant peaks
identify representative modes; their complex values retain both displacement
...
Small linear motion
  & Mode superposition and linear projection are valid. \\
Weak perspective
  & Small 3D displacement projects approximately linearly. \\
Well-spaced modes
  & One mode dominates near each resonant peak. \\
Broad-spectrum forcing
  & Relevant modes receive measurable excitation. \\
\end{tabularx}}
...
\begin{equation*}
  \begin{aligned}
    ...
    &\qquad\left\|\alpha_{v,k}J_{v,g}B_{c,g}\xi-
