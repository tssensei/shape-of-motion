...
\usepackage[paperwidth=30in,paperheight=40in,margin=0.62in]{geometry}
...
\newcommand{\postersection}[3]{%
  ...
  \colorbox{#3}{%
    \parbox{\dimexpr\linewidth-2\fboxsep\relax}{%
      \color{white}\bfseries\fontsize{38}{43}\selectfont
...
\newcommand{\postersubsection}[1]{%
  \vspace{0.10in}
  {\bfseries\color{PosterInk}\fontsize{29}{34}\selectfont #1\par}
  \vspace{0.03in}
}

\newcommand{\smalllabel}[1]{%
  {\bfseries\color{PosterSlate}\fontsize{18}{21}\selectfont\MakeUppercase{#1}}}

\newcommand{\notebox}[3]{%
  \vspace{0.08in}
  \noindent\fcolorbox{#1}{#2}{%
    \parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{%
      \vspace{0.05in}\textbf{#3}\vspace{0.05in}}}
  \vspace{0.08in}
}
    ...
    \IfFileExists{#1}{%
      ...
      \fcolorbox{PosterLine}{PosterFog}{%
        \parbox[c][#2][c]{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{%
          \centering
          {\bfseries\fontsize{23}{27}\selectfont Figure placeholder}\par
          \vspace{0.10in}
...
\newcommand{\pipelinebox}[4]{%
  \node[draw=#1,fill=#1!7,line width=1.8pt,rounded corners=3pt,
        align=center,text width=1.58in,minimum height=0.82in,#4] (#2)
        {\bfseries\fontsize{18}{21}\selectfont #3};}
...
\postersection{1.}{From 3DGS to dynamic 3D scenes}{PosterSlate}
...
\cite{kerbl2023gaussians}. Gaussian $g$ stores
\begin{equation*}
...
\notebox{PosterSlate}{PosterFog}{%
  \textbf{Why 3DGS is useful here.} The same representation provides sharp
  rendering, explicit world-space locations, foreground/background separation,
  rendered depth, and differentiable camera projection Jacobians.}

\postersubsection{How prior work makes 3DGS dynamic}

Dynamic reconstruction remains ill-posed because geometry, camera motion,
occlusion, and non-rigid object motion are entangled. Representative families
include:
...
\hline
Capture/model & Typical strategy \\
\hline
Synchronized multi-view or RGB-D
  & Triangulate changing geometry with simultaneous observations; accurate but
    hardware-heavy. \\
Dynamic NeRF
  & Learn time-dependent radiance or a canonical-to-observation deformation
    field, as in Nerfies/HyperNeRF \cite{park2021nerfies,park2021hypernerf}. \\
Dynamic 3DGS
  & Move or deform explicit Gaussians over time for fast rendering
    \cite{wu2024fourDgs,yang2024deformable}. \\
Persistent motion bases
  & Share a compact set of $SE(3)$ trajectories across Gaussians, as in Shape
    of Motion \cite{wang2025shape}. \\
\hline
\end{tabularx}}

Shape of Motion emphasizes that many monocular methods recover short-range
scene flow or canonical deformation, while synchronized multi-view systems
remain much easier to constrain. Our setting takes a different middle ground:
multiple views are available, but they are recorded asynchronously by one
phone.

\posterplaceholder{../poster_brief/figures/bush_static_3dgs.png}{2.50in}{%
  Canonical Bush 3DGS: high-detail appearance and explicit foreground centers.}

\postersection{2.}{Modal analysis: coupled motion, simple modes}{PosterSage}
...
Its undamped mode shapes satisfy
\begin{equation*}
  K\phi_i=\omega_i^2M\phi_i.
\end{equation*}
The mode vectors are linearly independent and $M$-orthogonal. Writing
$\Phi=[\phi_1,\ldots,\phi_N]$ transforms the coupled degrees of freedom into
...
Instead of tracking every coupled coordinate independently, modal analysis
keeps a small set of spatial patterns $\phi_i$ and lets scalar coordinates
$q_i(t)$ describe how strongly each pattern is active. Low-frequency modes
often dominate visible vibration, so the basis can be much smaller than the
original number of degrees of freedom.
...
\postersection{2.}{Modal analysis from ordinary video}{PosterSage}
...
video without knowing geometry or material parameters
\cite{davis2015imagespace}. Their key observation is that, near a resonant
...
\begin{tabularx}{\linewidth}{@{}>{\bfseries\color{PosterSlate}}p{0.28\linewidth}X@{}}
Small linear motion
...
\notebox{PosterSage}{PosterSand}{%
  \textbf{Important limit.} Davis recovers a useful \emph{projected} modal
  basis. It does not identify depth motion, 3D geometry, modal mass, stiffness,
  or a physically normalized eigenvector.}
...
\postersection{3.}{Synchronizing phase without synchronized time}{PosterClay}
...
\postersection{4.}{Our end-to-end pipeline}{PosterSlate}

\begin{center}
\begin{tikzpicture}[x=1in,y=1in,>=Latex]
  \pipelinebox{PosterSlate}{capture}{1. Sweep + fixed videos}{at={(0,1.18)}}
  \pipelinebox{PosterSlate}{colmap}{2. Joint COLMAP}{at={(2.02,1.18)}}
  \pipelinebox{PosterSage}{static}{3. Static 3DGS}{at={(4.04,1.18)}}
  \pipelinebox{PosterClay}{flow}{4. Flow + Fourier modes}{at={(6.06,1.18)}}
  \pipelinebox{PosterSage}{topology}{5. Pixel--Gaussian topology}{at={(6.06,0)}}
  \pipelinebox{PosterClay}{phase}{6. View phase sync}{at={(4.04,0)}}
  \pipelinebox{PosterSlate}{solve3d}{7. Local 3D solve}{at={(2.02,0)}}
  \pipelinebox{PosterSage}{fill}{8. Motion fill}{at={(0,0)}}
  \node[PosterMuted,below=0.16in of fill] (output)
        {\bfseries completed $\phi_k\in\mathbb C^{G\times3}$};
  ...
  \draw[->,PosterMuted,line width=1.6pt] (fill) -- (output);
\end{tikzpicture}
\end{center}
...
\posterplaceholder{../poster_brief/figures/bush_capture_and_cameras.png}{2.15in}{%
  Sweep frames, fixed references, and registered cameras around the canonical scene.}

\postersubsection{B. 2D frequency evidence and observation topology}
...
\postersubsection{C. Spatial structure, local solve, and completion}
...
\posterplaceholder{../poster_brief/figures/bush_rigid_components.png}{2.00in}{%
  Bush Gaussian graph colored by local component or observation support.}

\posterplaceholder{../poster_brief/figures/bush_modes_grid.png}{3.15in}{%
...
B. Kerbl et al. \emph{3D Gaussian Splatting for Real-Time Radiance Field
Rendering}. ACM TOG, 2023.
\bibitem{park2021nerfies}
K. Park et al. \emph{Nerfies: Deformable Neural Radiance Fields}. ICCV, 2021.
\bibitem{park2021hypernerf}
K. Park et al. \emph{HyperNeRF}. SIGGRAPH Asia, 2021.
\bibitem{wu2024fourDgs}
G. Wu et al. \emph{4D Gaussian Splatting for Real-Time Dynamic Scene Rendering}.
CVPR, 2024.
\bibitem{yang2024deformable}
Z. Yang et al. \emph{Deformable 3D Gaussians for High-Fidelity Monocular
Dynamic Scene Reconstruction}. CVPR, 2024.
\bibitem{wang2025shape}
Q. Wang et al. \emph{Shape of Motion: 4D Reconstruction from a Single Video}.
ICCV, 2025.
\bibitem{davis2015imagespace}
A. Davis, J. G. Chen, and F. Durand. \emph{Image-Space Modal Bases for Plausible
Manipulation of Objects in Video}. ACM TOG, 2015.
\end{thebibliography}}
