...
@dataclass(frozen=True)
class AnchorStructureGraphData:
    mode_index: int
    freq_hz: float
    graph_path: Path
...
def stable_uniform_edge_indices(edge_count: int, max_visible_edges: int) -> np.ndarray:
    if (
    ...
    return np.floor(
        np.linspace(0, edge_count, visible_count, endpoint=False, dtype=np.float64)
    ).astype(np.int64)
    ...
    for index, values in enumerate(choices):
        ...
        if np.any(mask):
            rgb[mask] = np.column_stack(
                [
                    np.full(np.count_nonzero(mask), values[0])
                    if np.isscalar(values[0])
                    else values[0][mask],
                    np.full(np.count_nonzero(mask), values[1])
                    if np.isscalar(values[1])
                    else values[1][mask],
                    np.full(np.count_nonzero(mask), values[2])
                    if np.isscalar(values[2])
                    else values[2][mask],
                ]
            )
...
def _anchor_graph_components(
    ...
    sizes = (
        np.bincount(components, minlength=int(components.max()) + 1).astype(np.int32)
        if num_anchors
...
def load_anchor_structure_graphs(
    manifest_path: str,
    modes: list[ModalModeData] | None = None,
    ...
    if not isinstance(modes_payload, list) or not modes_payload:
        raise ValueError(f"{manifest_path} does not contain non-empty modes list")
    if not all(isinstance(mode, dict) for mode in modes_payload):
        raise ValueError(f"{manifest_path} modes must contain mappings")
    graph_paths_present = ["anchor_graph_path" in mode for mode in modes_payload]
    if not any(graph_paths_present):
        return None
    ...
    if not all(graph_paths_present):
        raise ValueError(
            "Manifest modes must either all include anchor_graph_path or none do"
        )
    parameters = payload.get("parameters")
    if not isinstance(parameters, dict) or parameters.get("anchor_graph_enabled") is not True:
        raise ValueError(
            f"{manifest_path} has anchor graph paths without anchor_graph_enabled=true"
        )
    parameter_fields = {
    ...
    if missing_parameters:
        raise ValueError(
            f"{manifest_path} missing anchor graph parameters: {missing_parameters}"
        )
    ...
    if modes is None:
        modes = load_modal_modes(manifest_path)
    if len(modes) != len(modes_payload):
        raise ValueError("Anchor graph mode count does not match loaded modal modes")
    ...
    loaded_graphs: list[AnchorStructureGraphData] = []
    for mode_payload, mode in zip(modes_payload, modes):
        graph_path = _resolve_modal_artifact_path(
            manifest,
            mode_payload.get("anchor_graph_path"),
            "anchor_graph_path",
        )
        with np.load(str(graph_path), allow_pickle=False) as archive:
            ...
            graph = {key: np.asarray(archive[key]) for key in archive.files}

        def scalar(name: str) -> np.ndarray:
            value = graph[name]
            if value.shape != ():
                raise ValueError(f"{graph_path} {name} must be scalar")
            return value

        for parameter_name, artifact_name in parameter_fields.items():
            manifest_value = parameters[parameter_name]
            artifact_value = scalar(artifact_name).item()
            if isinstance(manifest_value, bool) or not isinstance(
                manifest_value, (int, float)
            ):
                raise ValueError(
                    f"{manifest_path} parameter {parameter_name} must be numeric"
                )
            if not np.isclose(
                ...
                raise ValueError(
                    f"{graph_path} {artifact_name} does not match {manifest_path}"
                )
        if not np.isclose(
            float(scalar("mad_scale").item()),
            1.4826,
            rtol=1e-6,
            atol=1e-8,
        ):
        ...
        for field_name, expected_value in expected_semantics.items():
            actual_value = scalar(field_name).item()
        ...
        if int(scalar("version").item()) != 1:
            raise ValueError(f"{graph_path} must be a version 1 anchor graph")
        if int(scalar("mode_index").item()) != mode.mode_index:
            raise ValueError(f"{graph_path} mode_index does not match manifest mode")
        if not np.isclose(
            float(scalar("freq_hz").item()), mode.freq_hz, rtol=1e-6, atol=1e-6
        ):
            raise ValueError(f"{graph_path} freq_hz does not match manifest mode")
        graph_checkpoint = scalar("source_checkpoint").item()
        if isinstance(graph_checkpoint, bytes):
            graph_checkpoint = graph_checkpoint.decode("utf-8")
        if graph_checkpoint != source_checkpoint:
            raise ValueError(f"{graph_path} source_checkpoint does not match manifest")
        num_gaussians = int(scalar("num_foreground_gaussians").item())
        if num_gaussians != mode.points_world.shape[0]:
            raise ValueError(
                f"{graph_path} foreground Gaussian count does not match modal points"
            )
        ...
        if anchor_indices.ndim != 1 or not np.issubdtype(anchor_indices.dtype, np.integer):
            raise ValueError(f"{graph_path} anchor_gaussian_indices must be 1-D integers")
        ...
        max_anchor_position_delta = (
            float(np.max(np.abs(points - mode.points_world[anchor_indices])))
            if num_anchors
            else 0.0
        )
        if max_anchor_position_delta > 1e-5:
            raise ValueError(
                f"{graph_path} anchor points differ from modal Gaussian positions by "
                f"max {max_anchor_position_delta:.6g}, above 1e-05"
            )

        view_ids = graph["view_ids"]
        if view_ids.ndim != 1:
            raise ValueError(f"{graph_path} view_ids must be 1-D")
        num_views = int(view_ids.shape[0])
        normalized_view_ids = tuple(str(np.asarray(value).item()) for value in view_ids)
        if len(set(normalized_view_ids)) != num_views:
        ...
        for name in (
            ...
            if graph[name].shape != (num_views,):
                raise ValueError(f"{graph_path} {name} must have shape ({num_views},)")
        ...
        if np.any(edges < 0) or np.any(edges >= num_anchors) or np.any(edges[:, 0] >= edges[:, 1]):
        ...
        if num_edges > 1:
            ...
            if not np.array_equal(expected_order, np.arange(num_edges)):
                raise ValueError(f"{graph_path} edge_index must be lexicographically sorted")
        edge_vectors = (
            "edge_distance",
            "edge_distance_weight",
            "edge_color_distance",
        ...
        for name in edge_vectors:
            if graph[name].shape != (num_edges,):
                raise ValueError(f"{graph_path} {name} must have shape ({num_edges},)")
        finite_metrics = (
            "edge_distance",
            "edge_distance_weight",
            "edge_color_distance",
        ...
        for name in finite_metrics:
            if not np.isfinite(graph[name]).all():
                raise ValueError(f"{graph_path} {name} must be finite")
        ...
        min_shared_views = int(scalar("min_shared_views").item())
        ...
        for name in ("edge_endpoint_gap_by_view", "edge_depth_jump_by_view"):
            if graph[name].shape != (num_edges, num_views):
                raise ValueError(f"{graph_path} {name} has invalid shape")
            if not np.isfinite(graph[name][support_mask]).all():
                raise ValueError(f"{graph_path} {name} is invalid on supporting views")

        expected_degree, expected_components, expected_sizes = _anchor_graph_components(
            num_anchors,
            edges,
        )
        degree = graph["degree"]
        ...
        if not np.array_equal(components, expected_components):
            raise ValueError(f"{graph_path} component_index is inconsistent with edge_index")
        if not np.array_equal(sizes, expected_sizes):
            raise ValueError(f"{graph_path} component_size is inconsistent with edge_index")
        if isolated.shape != (num_anchors,) or not np.array_equal(isolated, degree == 0):
            raise ValueError(f"{graph_path} isolated_mask is inconsistent with degree")
        if int(scalar("retained_edge_count").item()) != num_edges:
            raise ValueError(f"{graph_path} retained_edge_count is inconsistent")
        if int(scalar("anchor_count").item()) != num_anchors:
            raise ValueError(f"{graph_path} anchor_count is inconsistent")
        if int(scalar("isolated_anchor_count").item()) != int(isolated.sum()):
            raise ValueError(f"{graph_path} isolated_anchor_count is inconsistent")
        if int(scalar("component_count").item()) != sizes.shape[0]:
        ...
        loaded_graphs.append(
            AnchorStructureGraphData(
                ...
                anchor_gaussian_indices=anchor_indices.astype(np.int32),
                ...
                edge_view_support_count=support_count.astype(np.int32),
