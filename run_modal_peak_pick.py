...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p_analyze = sub.add_parser("analyze", help="Analyze a video or ordered image sequence once and write an immutable modal cache.")
    analyze_source = p_analyze.add_mutually_exclusive_group(required=True)
    analyze_source.add_argument("--video", help="Input video path.")
    analyze_source.add_argument("--image-dir", help="Ordered image-sequence directory.")
    p_analyze.add_argument("--frame-names-json", default=None, help="Ordered JSON list of image filename stems; required with --image-dir.")
    p_analyze.add_argument("--fps", type=float, default=None, help="Image-sequence FPS; required with --image-dir.")
    p_analyze.add_argument("--reference-frame", default=None, help="Explicit reference-frame stem; required with --image-dir.")
    p_analyze.add_argument("--source-identity", default=None, help="Optional immutable source identity recorded for ordered image input.")
    ...
    p_analyze.add_argument("--t0", type=float, default=None, help="Clip start time in seconds; video input only.")
